...
for d in ['ad2', 'ad3', 'csd']:
    # Load gradient image
    data = nb.load('/home/julia/data/gradients/results/repro/%s/%s_embed.nii.gz' %(d,d)).get_data()
    ...
    for t in range(6):
        ...
        io.save_mesh('/home/julia/data/gradients/results/repro/%s/%s_embed_viz_g%i_min.vtk'%(d,d,t),
                     {'faces':mesh['faces'], 'points':mesh['points'],
                       'data':data_mesh_min})
        io.save_mesh('/home/julia/data/gradients/results/repro/%s/%s_embed_viz_g%i_max.vtk'%(d,d,t),
                     {'faces':mesh['faces'], 'points':mesh['points'],
                       'data':data_mesh_max})
